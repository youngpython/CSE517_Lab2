...
function alloc=id3treehelper(xTr,yTr,maxdepth,weights,current_pos,isforest,avail_features,d)
    ...
    elseif length(unique(xTr','rows')) == 1 || maxdepth == 0 
        ...
        if isforest == true
            %FIXME Features are picked without replacement
            %{
            if length(avail_features) < ceil(sqrt(d))
                feature_indices = avail_features;
            else
                feature_indices = randsample(avail_features,ceil(sqrt(d)));
            end
            avail_features = setdiff(avail_features,feature_indices);
            %}
            %FIXME Features are picked with replacement
            %feature_indices = randsample(avail_features,ceil(sqrt(d)));
