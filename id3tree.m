...
T = zeros(7,n);
alloc = id3treehelper(xTr,yTr,maxdepth,weights,1);
...
function alloc=id3treehelper(xTr,yTr,maxdepth,weights,current_pos)
    ...
    elseif length(unique(xTr','rows')) == 1 || maxdepth == 0 
        ...
        alloc = [ id3treehelper(SL_x,SL_y,maxdepth-1,SL_weights,2*current_pos),... 
            [mode(yTr);feature;cut;2*current_pos;2*current_pos+1;floor(current_pos/2);current_pos],... 
            id3treehelper(SR_x,SR_y,maxdepth-1,SR_weights,2*current_pos+1)];   
    end
